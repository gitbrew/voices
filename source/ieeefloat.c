	...
	if (first == 0 && second == 0) {
		...
		if (expon == DEXP_MAX) {
			...
			if (expon == 0) {
				...
				mantissa = (first & 0x000FFFFF) + 0x00100000;
...
main()
{
	...
	TestFromIeeeSingle("7FC00100");
	TestFromIeeeSingle("7F800100");
...
#ifdef IEEE
	d[0] = 0x00800000L; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])));
	d[0] = 0x00400000L; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])));
	d[0] = 0x00000001L; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])));
	d[0] = 0x00000001L; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])) * 0.5);
	d[0] = 0x3F8FEDCBL; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])));
#if !(defined(sgi) || defined(NeXT))
	d[0] = 0x7FC00100L; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])));
	d[0] = 0x7F800100L; MaybeSwapBytes(d,4); TestToIeeeSingle(*((float*)(&d[0])));
#endif
#endif
	...
	TestFromIeeeDouble("7FF8002000000000");
	TestFromIeeeDouble("7FF0002000000000");
	...
	TestFromIeeeExtended("00000000000000000000");
	TestFromIeeeExtended("80000000000000000000");
	TestFromIeeeExtended("3FFF8000000000000000");
	TestFromIeeeExtended("BFFF8000000000000000");
	TestFromIeeeExtended("40008000000000000000");
	TestFromIeeeExtended("C0008000000000000000");
	TestFromIeeeExtended("7FFF0000000000000000");
	TestFromIeeeExtended("FFFF0000000000000000");
	TestFromIeeeExtended("7FFF8001000000000000");
	TestFromIeeeExtended("7FFF0001000000000000");
	TestFromIeeeExtended("3FFFFEDCBA9876543210");
...
#if defined(applec) || defined(THINK_C) || defined(METROWERKS)
	Hex2Bytes("7FFF00008001000000000000", bytes); TestToIeeeExtended(*((long double*)(bytes)));
	Hex2Bytes("7FFF00000001000000000000", bytes); TestToIeeeExtended(*((long double*)(bytes)));
