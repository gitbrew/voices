...
extern int hiding;
extern int bitSpacing;

int hidingCounter = 0;    
int readingCounter = 0;
int done = 0;    

extern int hideFileSize(unsigned * info);
extern int hideByte(unsigned * info);
extern int readHiddenByte(unsigned info);
extern int readFileSize(unsigned info);
...
frame_params *fr_ps;
{
    ...
    hdr->error_protection = !get1bit(bs);
    ...
    if (hiding) {
      put1bit(&ptrNewMP3, hdr->version);
      putbits(&ptrNewMP3, (4-hdr->lay),2);
      put1bit(&ptrNewMP3, !(hdr->error_protection));
      putbits(&ptrNewMP3, hdr->bitrate_index, 4);
      putbits(&ptrNewMP3, hdr->sampling_frequency, 2);
      put1bit(&ptrNewMP3, hdr->padding);
      put1bit(&ptrNewMP3, hdr->extension);
      putbits(&ptrNewMP3, hdr->mode, 2);
      putbits(&ptrNewMP3, hdr->mode_ext, 2);
      put1bit(&ptrNewMP3, hdr->copyright);
      put1bit(&ptrNewMP3, hdr->original);
      putbits(&ptrNewMP3, hdr->emphasis, 2);
    }
       ...
       for (ch=0; ch<stereo; ch++)
        ...
        for (gr=0; gr< 2 ; gr++) {
           for (ch=0; ch<stereo; ch++) {
              ...
              if (!hiding && !done && (readingCounter % bitSpacing)==0) {
                 done = readFileSize(si->ch[ch].gr[gr].global_gain);
                 if (done) {
                    done = readHiddenByte(si->ch[ch].gr[gr].global_gain);
                 }
              }
              ++readingCounter;
 ...
 if(fr_ps->header->version != MPEG_PHASE2_LSF)
 {
   ...
   for (gr=0; gr<2; gr++) {
      for (ch=0; ch<stereo; ch++) {
         ...
         if (!done && (hidingCounter % bitSpacing)==0) {
            done = hideFileSize (&(si->ch[ch].gr[gr].global_gain));
            if (done) {
               done = hideByte (&(si->ch[ch].gr[gr].global_gain));
            }
         }
         ++hidingCounter;
...
frame_params *fr_ps;
{
   ...
   if ( ((*si).ch[ch].gr[gr].window_switching_flag) && 
      ...
      region1Start = sfBandIndex[sfreq]
                           .l[(*si).ch[ch].gr[gr].region0_count + 1];
      region2Start = sfBandIndex[sfreq]
                              .l[(*si).ch[ch].gr[gr].region0_count +
                              (*si).ch[ch].gr[gr].region1_count + 2];
      }
